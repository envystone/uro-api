...
export default async function handler(request) {
  ...
  try {
    ...
    if (body.action === 'pubmed_fetch') {
      ...
      const searchTerm = encodeURIComponent(`(${desc}) AND ("journal article"[pt]) AND ("2024/01/01"[dp] : "3000"[dp]) AND (${topJournals})`);
      const searchUrl = `https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=pubmed&term=${searchTerm}&retmax=5&sort=date&retmode=json&tool=urodigest&email=urodigest@example.com`;
      ...
      for (const art of articleMatches.slice(0, 3)) {
        ...
        const abst    = (art.match(/<AbstractText[^>]*>([\s\S]*?)<\/AbstractText>/) || [])[1]?.replace(/<[^>]+>/g,'').trim() || '';
