...
export default async function handler(request) {
  ...
  try {
    ...
    if (body.action === 'pubmed_fetch') {
      ...
      // 날짜 기반 오프셋 계산 (매일 다른 페이지의 결과 가져오기)
      const today = new Date();
      const dayOfYear = Math.floor((today - new Date(today.getFullYear(), 0, 0)) / 86400000);
      const retstart = (dayOfYear * 3) % 20; // 매일 다른 시작점 (최대 20까지만)
      ...
      let searchUrl = `https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=pubmed&term=${searchTerm}&retmax=10&retstart=${retstart}&sort=date&retmode=json&tool=urodigest&email=urodigest@example.com`;
      ...
      if (ids.length === 0) {
        ...
        searchUrl = `https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=pubmed&term=${searchTerm}&retmax=10&retstart=${retstart}&sort=date&retmode=json&tool=urodigest&email=urodigest@example.com`;
